...
- [x] **5단계 — 시장(수요) 중심 시각화** (현재)
  ...
  수요 → 공급 6개 층의 22개 시장을 캐스케이드로 그리고, **병목(HBM·전력·CoWoS·
  EUV 등)을 펄스로 강조**한다. 데이터는 `data/markets/ai-semiconductor.json` 하나에
  모이며, 티커별 `narrative_score` 가 시장 노드별로 자동 집계된다. 시장 조사·갱신은
  `.claude/routines/market-research/` 루틴이 담당.
