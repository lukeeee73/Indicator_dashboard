...
| 주 단위 선행지수 | `USSLIND` | 성장 | — | Philly Fed. **2020-02 발표 중단 — 점수에서 자동 제외** |
...
### 발표가 끊긴 지표 (staleness 가드)

FRED 는 폐지된 시리즈도 과거 데이터를 계속 제공한다. 가드가 없으면 마지막 값이
"현재값" 으로 영원히 점수에 남는다 — 실제로 두 건이 그랬다.

- `USSLIND` 는 2020-02 이후 폐지(Philly Fed 가 state leading index 발표를 중단)
  됐는데도 가중치 2.0(성장 축의 25%)으로 계속 반영돼 **성장 점수를 +9.4 부풀리고**
  있었다.
- `KORCPIALLMINMEI`(2023-11 중단)는 한국 인플레 축의 **100%** 였다. 한국 탭의
  "현재 국면" 이 2023년 11월 판정을 그대로 보여주고 있었다.

그래서 `analyze.py` 는 **평가 시점 기준 `STALE_MONTHS`(12개월) 안에 관측이 없는
지표를 그 시점의 축 계산에서 제외**한다. 과거 시점을 걸어갈 때도 그 시점 기준으로
판단하므로 미래 정보가 새지 않는다. 분기·연간 시리즈의 정상적인 발표 지연
(GDP ~5개월, `FDHBFIN` ~11개월)은 통과한다.

결과는 `assessment.stale_excluded` 와 각 지표의 `current.stale` 에 기록되고,
대시보드는 이를 근거로 카드에 "갱신 중단" 배지를, 판정 카드 위에 경고를 띄운다.
축 하나가 통째로 비면(현재 한국 인플레) 숫자를 만들어내지 않고 **"판정 불가"** 로
표시한다.

> 참고: `backtest.py` / `compare_models.py` 는 자체 walk 를 돌며 이 가드를 공유하지
> 않는다. 사전 등록된 평가 산출물(`data/eval/`)을 결과를 보고 다시 만들지 않기
> 위해 손대지 않았다 — 다만 2020 이후 구간은 같은 사각지대를 갖고 있다.
...
- [x] **8단계 — 종목 검색 · 섹터 토글 · 가치 발굴 스크리닝** (현재)
  개별 종목을 스크롤이 아니라 **검색**(티커·회사명·섹터)으로 찾는다. 섹터 그룹은
  **기본 접힘 토글**로 바뀌고 카드(차트)는 열릴 때만 렌더 — 단, **뉴스 루틴이 오늘
  갱신한 종목이 있는 섹터**(`valuation.qualitative.as_of` == 오늘)만 그날 자동으로
  열리고 🗞️ 배지가 붙는다. 그리고 **가치 발굴 탭** (주식 탭 → 🎯 가치 발굴):
  버핏식 "훌륭한 기업을 훌륭한 가격에" 월간 스크리닝 — `scripts/value_screen.py` 가
  매월 1일(value-screen.yml) 전 종목 재무제표를 훑어 **사업의 질 3가지**(ROE ≥ 15% ·
  영업이익률 ≥ 15% · 4분기 연속 흑자) **× 가격 1가지**(적정가치 대비 20%+ 할인)를
  **모두** 통과한 기업만 `data/value_screen.json` 에 기록한다. 통과 기업은 카드
  모아보기 + 일반 카드에 🎯 배지, '기준 3개 통과(가격만 남은)' 후보는 지켜보기
  리스트로 표시. 통과 0개인 달이 정상 — 그것이 '기다림'이다.
