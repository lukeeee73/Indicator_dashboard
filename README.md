...
툴바의 **주제 필터**(반도체·AI칩 / 전력·에너지 / 제약·바이오 / AI·LLM /
투자·매크로 / 디자인)로 특정 주제의 노트만 밝게 볼 수 있다. 소속 판정은
① 태그·제목·경로·domain 키워드, ② `domain: ai`/`design` 같은 도메인 일치,
③ 시장 지도(`data/markets/*.json`)의 플레이어 티커 — 예: NVDA 뉴스 로그는
AI·반도체 지도 소속이므로 자동으로 "반도체·AI칩"에 묶인다. 주제 정의는
`app.js` 의 `WIKI_TOPICS` 에 있으며, 지식 레이어 프리셋·검색과 AND 로 조합된다.
