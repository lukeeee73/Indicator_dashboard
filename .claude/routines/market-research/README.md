...
- **옵시디언 시장 종합 파일** (`Luke_wiki/wiki/news/markets/{map_id}/{market_id}.md`):
  시장 노드별로 기업 동향 + 시장 구조·병목·뉴스를 한 페이지로 종합한 거울 레이어.
  daily 루틴이 [소속 기업 동향]을, 이 디렉토리의 루틴이 나머지 섹션을 갱신한다
  (각 산업 md 의 "옵시디언 시장 종합 파일 동기화" 절). 대시보드는 시장 노드
  클릭 시 이 파일을 지도 아래에 표시한다. 현재 운영: `ai-semiconductor/` (30개 노드).
  **글쓰기 문체는 `Luke_wiki/wiki/news/FORMAT.md` (투자 브리핑 v2, 2026-07-07~)** —
  쉬운 한국어 · "지금 상황→왜 중요→투자자 관점" 구조 · 용어 괄호 풀이.
